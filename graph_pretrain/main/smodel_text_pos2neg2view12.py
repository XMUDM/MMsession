...
class HeteroClassifier(nn.Module):
    ...
    def forward(self, g_list1, g_list2, graph_click, train=True):
        if train:
            loss1 = self.get_view1(g_list1, graph_click, train)
            loss2 = self.get_view2(g_list2, graph_click, train)
            return loss1, loss2
        else:
            g_emb1 = self.get_view1(g_list1, graph_click, train)
            g_emb2 = self.get_view2(g_list2, graph_click, train)
            return g_emb1, g_emb2

    def get_view1(self, g_list, graph_click, train=True):
        g_emb_list = []
        for g in g_list:
        ...
        if train:
            ...
            s2_ = self.Discriminator1(torch.cat([bt1, bt2], 1)).reshape(-1, col_num)
    ...
    def get_view2(self, g_list, graph_click, train=True):
        g_emb_list = []
        for g in g_list:
        ...
        if train:
            ...
            bt1 = anchor.repeat(1, col_num).reshape(-1, emb_dim)
            bt2 = session_memory.repeat(row_num, 1)
            s2_ = self.Discriminator2(torch.cat([bt1, bt2], 1)).reshape(-1, col_num)
...
def batcher2():
    def batcher_dev(batch):
        ...
        return dgl.batch(v1b1), dgl.batch(v1b2), dgl.batch(v2b1), dgl.batch(v2b2), c
...
if __name__ == "__main__":
    test = False
    save_path = config.save_path + "_text_pos2neg2view12"
    ...
    for category in categorys:
        ...
        path1 = "../data2_view1/{}/".format(category)
        ...
        for epoch in range(21):
            ...
            for v1b1, v1b2, v2b1, v2b2, c in train_loader:
                loss1, loss2 = model([v1b1, v1b2], [v2b1, v2b2], c, True)
                loss = loss1+loss2
                opt.zero_grad()
                loss.backward()
                opt.step()
            ...
            if epoch == 20:
                ...
                with torch.no_grad():
                    ...
                    for v1b1, v2b1 in tr_loader:
                        semb1,semb2 = model([v1b1], [v2b1], None, False)
                ...
                session_graph1 = joblib.load("../data2_view1/{}/graphs_test.bin".format(category))
                session_graph2 = joblib.load("../data2_view2/{}/graphs_test.bin".format(category))
            ...
            if epoch == 20:
                tg = time()
                data1 = joblib.load("../data2_view1/{}/graph_qid.bin".format(category))
                data2 = joblib.load("../data2_view2/{}/graph_qid.bin".format(category))
