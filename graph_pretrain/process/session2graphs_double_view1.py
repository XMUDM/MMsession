...
def session2graphs(path1, path2):
    # 1 read
    session = pd.read_csv(path1 + "session.csv", sep='\t', header=None, usecols=[0, 1, 3],
                          names=['session_id', 'session', 'split'])
    ...
    for se in session['session']:  # session
        ...
        for qid in s:  # query
            ...
            hg.nodes['img'].data['img_emb'] = torch.FloatTensor([query_img[qid] for qid in list(reindex_img_sim.keys())])
            qids.append(qid)
            gs.append(hg)
...
if __name__ == "__main__":
    for category in ['beauty', 'clothes', 'digital']:
        ...
        path2 = "../data_view1/{}/".format(category)
